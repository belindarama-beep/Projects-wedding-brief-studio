...
**What it buys:** roughly 2.7x more headroom before hitting a wall again. Given images are the driver, that's rough runway for more photos before the same failure mode returns — not a fix, a bigger buffer.

**Explicitly a stopgap, not a fix.** It doesn't change what's expensive, it just tolerates more of it. Doesn't preclude either path below — could be done today, immediately, independent of whatever gets built next.
...
**What it is:** split source material into smaller batches so no single call processes too many images at once — the thing the isolation test shows is actually expensive.
