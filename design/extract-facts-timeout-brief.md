...
**Sequencing: this must be built and validated before flag-resolution-carry-forward's resolutions matcher, not alongside or after it.** That matcher compares flags produced by the Flag pass across runs — which this work restructures from one call into two phases. Building it against today's single-call Flag output would mean building it again against the new shape once batching lands. Batching goes first. This does *not* apply to `flag-resolution-carry-forward-build-brief.md`'s separate `internal_only` interim fix (a UI toggle plus a reminder list, unrelated to the Flag pass's structure) — that one is unblocked and addresses a real privacy leak (see that brief), so it should not wait for batching just because it lives in the same document as the part that does.
...
## Three more design questions that must be answered before building

Same status as the one above: decided here, not left for the implementation to discover.

### Source deletion breaks the append-only rule as written — it needs its own answer, not an extension of the addition case

The append-only requirement above (§Batch composition) was written for *addition*: a new source item joining a not-yet-full batch, or starting a new one, without reshuffling where existing items already landed. Deletion is a different case, not a variant of it, and the existing rule doesn't actually cover it.

**Why it's different:** addition is unrelated new material — nothing about it should ripple backward into batches that don't contain it, which is exactly what "append-only" protects. Deletion removes material *from* a specific batch — that batch's facts genuinely need to change, because something they were derived from is gone. The question isn't whether to allow that batch to change (it must), it's how to do that without the change rippling into every *other* batch, which is what would actually violate the stability guarantee.

**The failure mode this rules out:** if batch membership is a recomputed function of "current source_items in insertion order, filled to capacity" — run fresh on every extraction — then deleting one item from the middle of an early, full batch shifts every subsequent item up one position, which can move items across batch boundaries throughout the entire project. That's a full reshuffle triggered by one deletion, exactly the instability the append-only rule exists to prevent, and it would silently reword unrelated, untouched facts for reasons that have nothing to do with those facts' own source material changing — the same failure mode carry-forward matching (`flag-resolution-carry-forward-build-brief.md`) is trying to avoid.

**The answer: batch membership must be a persisted, one-time assignment, not a recomputed function.** Concretely, a `batch_index` (or `batch_id`) column on `source_items`, set once when an item is first batched by insertion-ordered filling, and never recalculated from the current full list on a later run. Under this model:
- Deleting an item removes it from its batch. That batch's Phase 1 fact extraction re-runs (only that batch — the others are untouched, since their membership was never a function of the deleted item's presence). Phase 2 already re-runs fully over the current combined fact set on every extraction per the existing design, so it picks up the change with no extra work.
- **A batch that loses an item to deletion stays under-capacity permanently — it does not get backfilled from a later batch to top it back up.** Backfilling would mean pulling some later item into this batch, changing that item's own batch membership, which is the exact reshuffle being ruled out. The cost is that batches can drift below their nominal capacity over a project's lifetime as items get deleted; that's the accepted tradeoff for keeping the stability guarantee intact, not an oversight to fix later.

### Batch count ceiling — no number is set here, and one shouldn't be guessed

The brief already declines to guess the per-image cost curve's shape (linear vs. accelerating) rather than assert a number without evidence. The same discipline applies here: **no hard ceiling on batch count is proposed in this brief.** A specific cap (e.g., "batching breaks down past N batches") would be exactly the kind of unjustified number the per-image starting points elsewhere in this document were explicit about needing validation for, not asserting on paper.

What is a real, currently-unmeasured risk, and needs to be checked rather than assumed either way: Phase 2 runs once over the *complete combined* fact set from every batch, and is described above as "comparatively cheap... however large the source folder gets." That claim is about Phase 2 no longer paying the *per-image* cost — it is not evidence that Phase 2's cost stays flat as the combined *text* fact set itself grows large (a project with many batches means many more `extracted_items` feeding into one Phase 2 call). This is the same category of unmeasured claim the per-image cost curve was, just on the other side of the split, and deserves the same treatment rather than being assumed safe by association.

**What this means for the validation pass already required before batching replaces the current approach:** it must include a case with many batches (a project with meaningfully more source material than Arden & Theo, not just a repeat of the same-sized validation), specifically to observe Phase 2's timing as the combined fact set grows — not only to reconfirm contradiction detection still works, which is the validation already planned. If that test shows Phase 2 itself approaching the wall-clock ceiling at some volume, *that's* the point where a real ceiling (or a further split of Phase 2 itself) becomes a decision to make — on measured evidence, the same standard applied to the image-count question throughout this document, not predicted now.

A related, softer signal worth naming without committing to a mechanism: whatever batch count actually turns out to be uncomfortable is also the point where "click Run extraction and wait" stops being a reasonable interaction regardless of whether it technically still finishes inside the wall-clock limit — which is what the next section is about.

### Progress feedback should mirror the batch/phase structure, not invent its own

Today's Extract is one call and one spinner until it succeeds or times out — tolerable at 150s, worse at whatever a many-batch project's total wall-clock time becomes even if each individual call stays safely under its own ceiling. Batching creates real, already-decided checkpoints (each Phase 1 batch completing; Phase 2 starting; Phase 2 completing) that don't exist today. The design principle: **progress feedback should be a direct reflection of those checkpoints, not a separately invented UI concept that then needs its own fake granularity.** A progress bar that estimates "60% done" without a real event behind that number would be inventing exactly the kind of unearned precision this document has been deliberately avoiding elsewhere (the per-image cost curve, the batch count ceiling above) — it should not be built here either.

**What surfacing the real checkpoints requires, left as a build-time choice rather than decided here:** the client needs some way to learn about each batch/phase completing as the run progresses, which needs either (a) polling a status column on the `extractions` row (or a new per-batch status table) that Phase 1/Phase 2 update as they go, (b) a Supabase Realtime subscription on that same state instead of polling, or (c) a streaming response from the edge function itself, avoiding new schema but keeping the client connected for the whole run — which reintroduces some of the single-long-call fragility (a dropped connection mid-run needs its own recovery story) that batching was meant to get away from. Each has a real tradeoff and none is chosen here.

**What is decided:** whichever mechanism is picked, the checkpoints it surfaces must be the actual batch and phase boundaries the implementation already knows precisely ("batch 2 of 5, text," "batch 4 of 5, images," "finding contradictions across everything found so far") — not boundaries invented after the fact to make a progress bar look smooth. This should be designed as part of the same build as batching itself, not bolted on afterward, because the checkpoints it needs to show are exactly the ones the batching architecture is already creating — a later retrofit would mean guessing at boundaries the implementation would have already known exactly.

**Mechanism actually chosen, at build time: client-driven, no new status infrastructure.** The client itself calls each Phase 1 batch sequentially, then Phase 2 — so every checkpoint above is something it already knows the instant that call's own `invoke` resolves, with no polling, no Realtime subscription, and no streaming response needed. This sidesteps the tradeoff this section raised, rather than picking one of (a)/(b)/(c): those are solutions for surfacing server-side progress to a client that isn't driving the sequence itself. Here the client is what's driving it.

## Implementation note: interrupted runs now leave a visible trace, on purpose

The single-call design failed atomically — a 145s call that errored left nothing behind. The batched design does not: five sequential calls over several minutes mean a tab closed mid-run leaves `extracted_items` already written under an `extraction_id` that never reaches Phase 2. This is not a correctness problem (nothing reads an incomplete extraction's rows), but left undiagnosed it would accumulate silently, project after project, with no way to tell an abandoned run from a slow one after the fact.

**Decided:** `extractions` gained a `status` column (`'running'` | `'complete'`), set on creation and flipped to `'complete'` by `flag-facts` only after it successfully writes flags. An extraction stuck at `'running'` is diagnosable as abandoned rather than a mystery. No automatic cleanup of orphaned rows is built — consistent with this document's working rule that extraction outputs are keep-by-default — this is purely about making the abandoned state legible, not about removing it.
